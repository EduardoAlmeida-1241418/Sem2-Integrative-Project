...
/**
 * This class benchmarks the execution time of the maintenance algorithm
 * for different railway network scenarios. It loads station and line data from CSV files,
 * runs the maintenance algorithm, and writes the results to a CSV file.
 */
public class US14ExecutionTime {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String resultsFile = resultsDir + "/US14_results.csv";
        ...
        for (Path stationFile : stationFiles) {
            ...
            // Create the controller for the maintenance algorithm
            MaintenanceRailwayLineController controller =
                    new MaintenanceRailwayLineController(stations, lines, baseName);

            // Set and filter all railway lines
            controller.setRailwayLinesType("All Railway Lines");
            controller.filterRailwayLinesType();

            // If the graph is not connected or not Eulerian, register NA and continue
            if (controller.isNotGraphConnected() || controller.setEulerianPathType()) {
                outputLines.add(String.format("%s,%d,NA", baseName, stations.size()));
                continue;
            }

            // Prepare valid stations and calculate degrees
            controller.setValidStationsConnectedLines();
            controller.calculateVertexDegrees();

            // Get valid initial stations
            List<Station> validInitialStations = controller.getValidInitialStations();
            if (validInitialStations.isEmpty()) {
                outputLines.add(String.format("%s,%d,NA", baseName, stations.size()));
                continue;
            }
            controller.addStationToStartingStations(validInitialStations.get(0));

            // Measure the execution time of the maintenance algorithm
            ...
            outputLines.add(String.format(Locale.US, "%s,%d,%.3f", baseName, stations.size(), timeSeconds));
        }
