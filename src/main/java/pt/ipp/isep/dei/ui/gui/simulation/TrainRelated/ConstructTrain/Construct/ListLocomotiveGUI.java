...
/**
 * GUI class responsible for displaying and managing the list of locomotives
 * available for selection in the train construction process.
 */
public class ListLocomotiveGUI implements Initializable {

    private String unknownImagePath = "/images/Locomotive/UnknownImageLocomotive.png";
    private ListLocomotiveController controller;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        controller = new ListLocomotiveController();
    }
    ...
    /**
     * Initializes the GUI components and binds the table columns to locomotive properties.
     */
    ...
    /**
     * Handles the event when the user selects a locomotive and proceeds to the next step.
     *
     * @param event The action event.
     */
    @FXML
    public void handleSelectLocomotiveButton(ActionEvent event) {
        if (listLocomotiveTableView.getSelectionModel().getSelectedItem() == null) {
            errorLabel.setText("You need to select a Locomotive first!!");
            return;
        }

        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/trainRelated/ConstructTrainRelated/ConstructTrain/ListAvailableCarriage.fxml"));
            Parent root = loader.load();

            ListCarriageGUI listCarriageGUI = loader.getController();
            listCarriageGUI.setData(
                    controller.getSimulation(),
                    (Locomotive) listLocomotiveTableView.getSelectionModel().getSelectedItem(),
                    (Stage) ((Node) event.getSource()).getScene().getWindow()
            );
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.setTitle("MABEC - Simulation: List Available Carriages");
    ...
    /**
     * Handles the event when the user cancels the locomotive selection and returns to the previous screen.
     *
     * @param event The action event.
     */
    @FXML
